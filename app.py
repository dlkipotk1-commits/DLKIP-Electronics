...
.video-grid {{ display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:14px; }}
.video-card {{
    overflow:hidden; border:1px solid rgba(255,77,0,.32); border-radius:18px; background:#090909;
    box-shadow:0 0 24px rgba(255,55,0,.06);
}}
.video-title {{ padding:9px 12px; color:var(--orange2); font-size:11px; font-weight:900; letter-spacing:.7px; text-align:center; }}
.video-card video {{ display:block; width:100%; aspect-ratio:16 / 9; height:auto; object-fit:cover; background:#050505; }}
...
.bot-avatar {{ width:32px; height:32px; border-radius:7px; display:flex; align-items:center; justify-content:center; background:linear-gradient(135deg,#ff6200,#e62d00); font-size:17px; box-shadow:0 0 16px rgba(255,70,0,.2); }}
...
<section class="video-grid">
    <div class="video-card">
        <div class="video-title">DLKIP • BLACK PCB</div>
        <video src="/static/black.mp4" autoplay muted loop playsinline preload="metadata"></video>
    </div>
    <div class="video-card">
        <div class="video-title">DLKIP • ORANGE PCB</div>
        <video src="/static/orange.mp4" autoplay muted loop playsinline preload="metadata"></video>
    </div>
</section>
